...
<main>
    <div class="container-fluid">
        ...
        <header id="main-header">
            <h1>Cuti</h1>
            <p>Halaman ini menangani data terkait cuti</p>
            <button class="btn btn-sm btn-header btn-modal" id="create-vacation-form"><span class="glyphicon glyphicon-pencil"></span> Tambahkan data</button>
        </header>

        <div class="sub-header"> 
            <form action="/form/cuti" method="GET" style="display:inherit">    
                ...
                <div class="search" id="date-based" style="position:relative">
                    <button type="button" class="btn btn-default" id="btn-date-based">TANGGAL CUTI</button>
                    <div class="form-group" style="position: absolute;left: 50%;margin-top: 5px;transform: translateX(-50%);z-index: 5;display: none;width: 400px;">
                        <div class="row">
                            <div class="col-md-6">
                                <input type="date" name="vacation_date_start" class="form-control">
                            </div>
                            <div class="col-md-6">
                                <input type="date" name="vacation_date_end" class="form-control">
                            </div>
                        </div>
                    </div>
                </div> 
        ...
        <div class="main-data" style="background:var(--theme-color-grey)">
            <?php echo count($vacationData)<1?'<div class="text-center">Belum terdapat data tersimpan</div>':''; ?>
            <div class="container-fluid grid-view">
                <?php foreach($vacationData as $data): ?>
                    <a href="/form/cuti/detail?v=<?= $data->id ?>">
                    <div class="cover-grid" style="overflow-y:auto;">
                        <ul>
                            <li><span class="glyphicon glyphicon-user"></span> <?= $data->submitter; ?></li>
                            <li><span class="glyphicon glyphicon-tasks"></span> <?= ucfirst($data->requisite); ?></li>
                            <li><span class="glyphicon glyphicon-calendar"></span> <?= ucfirst($data->day_used); ?> hari</li>
                            <?php $vdate = explode('<br>', $data->vacation_date); ?>
                            <?php if(count($vdate)>3): ?>
                                <li><?= $vdate[0]; ?></li>
                                <li><?= $vdate[1]; ?></li>
                                <li>...</li>
                                <li><?= $vdate[count($vdate)-1]; ?></li>
                            <?php else: ?>
                                <li><?= $data->vacation_date; ?></li>
                            <?php endif; ?>
                        </ul>
                    </div>
                    </a>
    ...
    <div class="app-form modal" id="modal-create-vacation-form">         
        <div class="modal-content modal-wizard show">
            <div class="modal-header">
                <h3>Tambahkan Data Cuti</h3>
            </div>

            <div class="description">
                <p>Form ini digunakan untuk menambahkan data cuti.</p>
                <dl>
                <dt>Catatan</dt>
            ...
            <form action="/form/cuti/create" method="post">
                <div class="form-group">
                ...
                <div class="form-group inline-input vacation-date">
                    <label>Tanggal</label>
                ...
                <!-- <div class="form-group">
                    ...
                    <span class="badge" id="count-days" style="margin-left:20px">1</span>
                </div> -->
